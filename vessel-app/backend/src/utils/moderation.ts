...
const afrikaansAndZulu = ['bliksem', 'poes', 'doos', 'naai', 'sukkel', 'moer', 'voetsek', 'inyoko', 'hamba uyofa']
...
function termToPattern(term: string): RegExp | null {
  ...
  const wordPatterns = normalized.split(' ').filter(Boolean).map((word) => word.split('').map((char) => `${char}+`).join('[\\s]*'))
